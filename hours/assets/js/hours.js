...
function printLibraries(libraryData) {
	//sort list of libraries array by alpha
	var libraryData  = sortByKey(libraryData, 'library_name');
	
	//loop through library list array, print templates
	$.each(libraryData[0], function(key,objectData) {
...
function printLibraryTemplate(key,objectData) {
	...
	var libtemp = _.template('\
		<li class="list-group-item library-item-hrs">\
...
function openStatusBadge(x,y) {
	if ( x == true ) {
		$("#libOpen" + y).html('<span class="badge-green badge">Open</span>');	
	} else if ( x == false ){
		$("#libOpen" + y).html('<span class="badge-red badge">Closed</span>');
	}
}
...
$(document).ready(function () {
	$("#search-field").on("keyup click input", function () {
		if (this.value.length >= 0) {
			$(".library-item-hrs").show().filter(function () {
